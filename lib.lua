...
local function sendWebhook(status, waveNumber)
	task.spawn(function()
		...
		local payload = HttpService:JSONEncode({
			...
			embeds     = {{
				...
				fields = {
					...
					{ name = "⏳ Wave Reached", value = "`" .. tostring(waveNumber or "Unknown") .. "`", inline = true }
				},
				footer = {
					text = "Match Tracker System • Account: " .. LocalPlayer.DisplayName
				}
			}}
		})
		...
		if not success then
			warn("⚠️ Failed to send webhook request: " .. tostring(response))
		elseif response and response.StatusCode ~= 204 and response.StatusCode ~= 200 then
			warn("❌ Discord API Error [" .. tostring(response.StatusCode) .. "]: " .. tostring(response.Body))
		end
	end)
end
...
local function GetCurrentWave()
	...
	local ok, container = pcall(function()
		return display.Frame.wave.container
	end)
	...
	local waveNum = valLabel.Text:match("(%d+)")
...
task.spawn(function()
	...
	while true do
		...
		if status and not hasSent then
			hasSent = true
